...
async function loadTable() {
  ...
  let tbody =
    ...
    document.body.appendChild(document.createElement("tbody"));
  let Weekday_Array = [];
  let Rainfall_Array = [];
  ...
  for (let i = 0; i < fetchData.weatherForecast.length; i++) {
    if (
      Weekday_Array.length > i &&
      Temperature_Array.length > i &&
      Rainfall_Array.length > i &&
      PublicHoliday_Array.length > i
    ) {
      newArrays.push([
        Weekday_Array[i],
        Temperature_Array[i],
  ...
  // http://localhost:8000/
  await fetch("https://ai.impetus-go.me/", {
    method: "POST",
    headers: {
    ...
    .then(async (response) => response.json()) // Parse the JSON from the response
    .then(async (data) => {
      ...
      if (data && Array.isArray(data["Predicted demands"])) {
        ...
        console.log(
          "No 'Predicted demands' array found or the property is not an array."
        );
      }
      ...
      secondTableBody = await handleTemp(secondTableBody, 'Avg Temp (˚C)', 'temperature')
      secondTableBody =   await handleTemp(secondTableBody, 'Rainfall', 'rainfall')
      secondTableBody =   await handleTemp(secondTableBody, 'Amount', 'demand')
    
    //   const tempRow3 = await handleTemp('Amount', 'temperature')
    //   const tempRow3 = await handleTemp('Amount', 'temperature')
    // //   const tempRow = await handleTemp('Avg Temp (˚C)', 'temperature')

    //   secondTableBody.appendChild(tempRow);
...
async function handleTemp(secondTableBody, key, value) {
    let records = await fetch("/history?min=5&max=40&holiday=yes");
    let dataObjects = await records.json();
    dataObjects = dataObjects.result
    ...
    for(data of dataObjects) {
        ...
        headerCell2.textContent = data[value];
        row.appendChild(headerCell2);
    }
    secondTableBody.appendChild(row);

    return secondTableBody
}
loadTable();
